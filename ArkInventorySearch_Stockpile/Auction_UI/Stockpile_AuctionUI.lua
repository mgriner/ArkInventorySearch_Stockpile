...
local BROWSE_PARAM_RARITY_TABLE = 9;
local PRICE_TYPE_UNIT = 1;
local PRICE_TYPE_STACK = 2;

UIPanelWindows["StockpileFrame"] = { area = "doublewide", pushable = 0, width = 840 };

StockpileFrame_Sort = { list = { existingSortColumn = "name", existingSortReverse = false }, bidder = { existingSortColumn = "name", existingSortReverse = false }, owner = { existingSortColumn = "name", existingSortReverse = false }, }
StockpileBrowseRarityFilters = {}

MoneyTypeInfo["AUCTION_DEPOSIT"] = {
	UpdateFunc = function()
		-- if ( not StockpileFrameAuctions.duration ) then
			-- StockpileFrameAuctions.duration = 0
		-- end
		-- return CalculateAuctionDeposit(StockpileFrameAuctions.duration);
	end,
	collapse = 1,
};
...
StaticPopupDialogs["BUYOUT_AUCTION"] = {
	...
	OnAccept = function(self)
		PlaceAuctionBid(StockpileFrame.type, GetSelectedAuctionItem(StockpileFrame.type), StockpileFrame.buyoutPrice);
	end,
	OnShow = function(self)
		MoneyFrame_Update(self.moneyFrame, StockpileFrame.buyoutPrice);
	end,
...
StaticPopupDialogs["BID_AUCTION"] = {
	...
	button2 = CANCEL,
	OnAccept = function(self)
		-- PlaceAuctionBid(StockpileFrame.type, GetSelectedAuctionItem(StockpileFrame.type), MoneyInputFrame_GetCopper(BrowseBidPrice));
	end,
	OnShow = function(self)
...
StaticPopupDialogs["CANCEL_AUCTION"] = {
	...
	OnShow = function(self)
		-- MoneyFrame_Update(self.moneyFrame, StockpileFrameAuctions.cancelPrice);
		-- if ( StockpileFrameAuctions.cancelPrice > 0 ) then
...
function StockpileFrame_OnLoad (self)
	...
	-- MoneyInputFrame_SetPreviousFocus(BrowseBidPrice, StockpileBrowseMaxLevel);
	-- MoneyInputFrame_SetNextFocus(BrowseBidPrice, StockpileBrowseName);
	...
	-- StockpileFrameBid.page = 0;
	-- FauxScrollFrame_SetOffset(BidScrollFrame,0);
	-- GetBidderAuctionItems(StockpileFrameBid.page);

	-- StockpileFrameAuctions.page = 0;
	-- FauxScrollFrame_SetOffset(AuctionsScrollFrame,0);
	-- GetOwnerAuctionItems(StockpileFrameAuctions.page);
	
	MoneyFrame_SetMaxDisplayWidth(StockpileFrameMoneyFrame, 160);
	
	-- Init rarity filters
	StockpileFrameBrowse_PopulateRarity()
	
end

function StockpileFrame_Show()
	if (IsKioskModeEnabled()) then
		UIErrorsFrame:AddMessage(ERR_CLIENT_LOCKED_OUT, 1.0, 0.1, 0.1, 1.0);
		CloseAuctionHouse();
		HideUIPanel(StockpileFrame);
		return;
	end

	if ( StockpileFrame:IsShown() ) then
		StockpileFrameBrowse_Update();
		--StockpileFrameBid_Update();
		--StockpileFrameAuctions_Update();
	else
		ShowUIPanel(StockpileFrame);
		...
		if ( not StockpileFrame:IsShown() ) then
			CloseAuctionHouse();
		end
	end
end

function StockpileFrame_Hide()
	HideUIPanel(StockpileFrame);
end

function StockpileFrame_OnShow (self)
	self.gotAuctions = nil;
	self.gotBids = nil;
	StockpileFrameTab_OnClick(StockpileFrameTab1);
...
function StockpileFrameTab_OnClick(self, button, down, index)
	local index = self:GetID();
	PanelTemplates_SetTab(StockpileFrame, index);
	-- StockpileFrameAuctions:Hide();
	StockpileFrameBrowse:Hide();
	-- StockpileFrameBid:Hide();
	...
	elseif ( index == 2 ) then
		-- Bids tab
		StockpileFrameTopLeft:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Bid-TopLeft");
		StockpileFrameTop:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-Top");
		StockpileFrameTopRight:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-TopRight");
		StockpileFrameBotLeft:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Bid-BotLeft");
		StockpileFrameBot:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-Bot");
		StockpileFrameBotRight:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Bid-BotRight");
		-- StockpileFrameBid:Show();
		StockpileFrame.type = "bidder";
		SetAuctionsTabShowing(false);
	elseif ( index == 3 ) then
		-- Auctions tab
		StockpileFrameTopLeft:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-TopLeft");
		StockpileFrameTop:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-Top");
		StockpileFrameTopRight:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-TopRight");
		StockpileFrameBotLeft:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-BotLeft");
		StockpileFrameBot:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-Bot");
		StockpileFrameBotRight:SetTexture("Interface\\AuctionFrame\\UI-AuctionFrame-Auction-BotRight");
		-- StockpileFrameAuctions:Show();
...
function StockpileFrameBrowse_OnLoad(self)
	self:RegisterEvent("AUCTION_ITEM_LIST_UPDATE");

	-- set default sort
	--StockpileFrame_SetSort("list", "quality", false);
	StockpileFrame_SetSort("list", "name", false);
end

function StockpileFrameBrowse_OnShow()
	StockpileFrameBrowse_Update();
	StockpileFrameFilters_Update();
end

function StockpileFrameBrowse_UpdateArrows()
	SortButton_UpdateArrow(StockpileBrowseNameSort, "list", "quality");
	SortButton_UpdateArrow(StockpileBrowseLevelSort, "list", "level");
	SortButton_UpdateArrow(StockpileBrowseDurationSort, "list", "duration");
	SortButton_UpdateArrow(StockpileBrowseHighBidderSort, "list", "seller");
	SortButton_UpdateArrow(StockpileBrowseCurrentBidSort, "list", "unitprice");
end

function ArkInventorySearch_Stockpile:EVENT_STOCKPILE_CACHE_ITEM_LIST_UPDATE( )
	print("EVENT_STOCKPILE_CACHE_ITEM_LIST_UPDATE fired!!!!!!!!!!!!!!!!!!!")
	print("isSearching: " .. tostring(ArkInventorySearch_Stockpile.StockpileFrameBrowse_isSearching))
	StockpileFrameBrowse_Update();
	...
	StockpileFrameBrowse_UpdateArrows();
	print("isSearching: " .. tostring(ArkInventorySearch_Stockpile.StockpileFrameBrowse_isSearching))
end

-- function StockpileFrameBrowse_OnEvent(self, event, ...)
...
function GetSearchTableItemLink( index )
	return ArkInventory.Search.SourceTable[index].h or ""
end

function StockpileBrowseButton_OnClick(button)
	assert(button);
	
	if ( GetCVarBool("auctionDisplayOnCharacter") ) then
		if ( not DressUpItemLink(GetSearchTableItemLink(button:GetID() + FauxScrollFrame_GetOffset(StockpileBrowseScrollFrame))) ) then
			DressUpBattlePet(GetAuctionItemBattlePetInfo("list", button:GetID() + FauxScrollFrame_GetOffset(StockpileBrowseScrollFrame)));
		end
	end
	SetSelectedAuctionItem("list", button:GetID() + FauxScrollFrame_GetOffset(StockpileBrowseScrollFrame));
	-- Close any auction related popups
	-- CloseAuctionStaticPopups();
	StockpileFrameBrowse_Update();
end

function StockpileBrowseDropDown_OnLoad(self)
	UIDropDownMenu_Initialize(self, StockpileBrowseDropDown_Initialize);
...
-- temp func theres better way to do this
function StockpileFrameBrowse_PopulateRarity()
...
function StockpileBrowseDropDown_Initialize()
	local info = UIDropDownMenu_CreateInfo();
	info.text = ALL;
	info.colorCode = "|cffffd100";
	info.value = -1;
	info.func = StockpileBrowseDropDown_OnClick;
	...
	for i=0, getn(ITEM_QUALITY_COLORS)  do
		...
		info.keepShownOnClick = true;
		UIDropDownMenu_AddButton(info);
		--StockpileBrowseRarityFilters[i] = false;
	end
end

function StockpileBrowseDropDown_OnClick(self, arg1, arg2, checked)
	StockpileBrowseRarityFilters[self.value] = not StockpileBrowseRarityFilters[self.value]
	--UIDropDownMenu_SetSelectedValue(StockpileBrowseDropDown, self.value);
	if self.value == -1 then
		StockpileBrowseDropDownToggleAll( StockpileBrowseRarityFilters[self.value] )
	...
	ToggleDropDownMenu(1, nil, StockpileBrowseDropDown)
	StockpileFrameBrowse_Search()
end

function StockpileBrowseDropDownToggleAll( allChecked )
	for index, isChecked in pairs( StockpileBrowseRarityFilters ) do
		StockpileBrowseRarityFilters[index] = allChecked
	end
end

function StockpileFrameBrowse_Reset(self)
	...
	StockpileBrowseDurationSort:Show();
	StockpileBrowseHighBidderSort:Show();
	StockpileBrowseCurrentBidSort:Show();
	...
	StockpileBrowseLevelSort:SetText(StockpileFrame_GetDetailColumnString(StockpileFrameBrowse.selectedCategoryIndex, StockpileFrameBrowse.selectedSubCategoryIndex));
	StockpileFrameFilters_Update()
...
function StockpileBrowseResetButton_OnUpdate(self, elapsed)
	if ( (StockpileBrowseName:GetText() == "") and (StockpileBrowseMinLevel:GetText() == "") and (StockpileBrowseMaxLevel:GetText() == "") and
	     ...
	     (not StockpileIsUsableCheckButton:GetChecked()) and (not StockpileExactMatchCheckButton:GetChecked()) and (UIDropDownMenu_GetSelectedValue(StockpileBrowseDropDown) == -1) and
...
function StockpileFrame_SortStockpileSetSort(sortTable, sortColumn, sortReverse)
	if not ArkInventory.Search.SourceTable then
		print("no search table...")
		return
	end
	if sortTable == "list" and sortColumn == "name" then
		if sortReverse then
...
function StockpileFrame_SetSort(sortTable, sortColumn, oppositeOrder)
	-- clear the existing sort.
	--SortAuctionClearSort(sortTable);
	StockpileFrame_SetStockpileSort(sortTable, sortColumn, oppositeOrder)
	
	-- set the columns
	for index, row in pairs(AuctionSort[sortTable.."_"..sortColumn]) do
		if (oppositeOrder) then
			StockpileFrame_SortStockpileSetSort(sortTable, row.column, not row.reverse);
		else
			StockpileFrame_SortStockpileSetSort(sortTable, row.column, row.reverse);
		end
	end
	ArkInventorySearch_Stockpile:SendMessage( "EVENT_STOCKPILE_CACHE_ITEM_LIST_UPDATE")
end

function StockpileFrame_GetStockpileSort(sortTable)
	if sortTable == "list" then
		return StockpileFrame_Sort[sortTable].existingSortColumn, StockpileFrame_Sort[sortTable].existingSortReverse;
	end
end

function StockpileFrame_SetStockpileSort(sortTable, sortColumn, sortReverse)
	StockpileFrame_Sort[sortTable].existingSortColumn = sortColumn
	StockpileFrame_Sort[sortTable].existingSortReverse = sortReverse
end

function StockpileFrame_OnClickSortColumn(sortTable, sortColumn)
	-- change the sort as appropriate
	--local existingSortColumn, existingSortReverse = GetAuctionSort(sortTable, 1);
	local existingSortColumn, existingSortReverse = StockpileFrame_GetStockpileSort(sortTable);
	...
	StockpileFrame_SetSort(sortTable, sortColumn, oppositeOrder);
...
local function StockpileFrameBrowse_SearchHelper(...)
	...
	-- using -2 we can no whether or not there is no editbox entry
	...
	if ( not prevBrowseParams ) then
		...
		for i = 1, select('#', ...) do
			if ( i == BROWSE_PARAM_RARITY_TABLE ) then
				for index, isChecked in pairs( rarity ) do
					if isChecked ~= prevBrowseParams[i] then
						should_update = true
						should_filter = true
						break
					end
		...
		-- check rarity filters separately because it is a table
		for index, isChecked in pairs(rarity) do
	...
	if should_filter then
		...
		local sortColumn = StockpileFrame_Sort[sortTable].existingSortColumn
		local sortReverse = StockpileFrame_Sort[sortTable].existingSortReverse
		StockpileFrame_SetSort( sortTable, sortColumn, sortReverse )
	end
...
function StockpileFrameBrowse_Search()
	if (StockpileFrame_DoesCategoryHaveFlag("WOW_TOKEN_FLAG", StockpileFrameBrowse.selectedCategoryIndex)) then
...
function StockpileBrowseSearchButton_OnUpdate(self, elapsed)
	...
	--if ( CanSendAuctionQuery("list") ) then
		...
		-- end
		StockpileBrowseNameSort:Enable();
		StockpileBrowseLevelSort:Enable();
		StockpileBrowseDurationSort:Enable();
		StockpileBrowseHighBidderSort:Enable();
		StockpileBrowseCurrentBidSort:Enable();
		StockpileFrameBrowse_UpdateArrows();
	else
		self:Disable();
		-- BrowsePrevPageButton:Disable();
		-- BrowseNextPageButton:Disable();
		StockpileBrowseNameSort:Disable();
		StockpileBrowseLevelSort:Disable();
		StockpileBrowseDurationSort:Disable();
		StockpileBrowseHighBidderSort:Disable();
		StockpileBrowseCurrentBidSort:Disable();
	end
...
function StockpileFrameFilters_Update(forceSelectionIntoView)
	StockpileFrameFilters_UpdateCategories(forceSelectionIntoView);
	-- Update scrollFrame
	FauxScrollFrame_Update(StockpileBrowseFilterScrollFrame, #OPEN_FILTER_LIST, NUM_FILTERS_TO_DISPLAY, BROWSE_FILTER_HEIGHT);
end

function StockpileFrameFilters_UpdateCategories(forceSelectionIntoView)
	...
	for categoryIndex, categoryInfo in ipairs(AuctionCategories) do
		...
		if ( selected ) then
			StockpileFrameFilters_AddSubCategories(categoryInfo.subCategories);
		end
	end
	...
	for i = 1, NUM_FILTERS_TO_DISPLAY do
		...
		if ( dataIndex <= #OPEN_FILTER_LIST ) then
			...
			if ( info ) then
				StockpileFilterButton_SetUp(button, info);
...
function StockpileFrameFilters_AddSubCategories(subCategories)
	if subCategories then
		for subCategoryIndex, subCategoryInfo in ipairs(subCategories) do
			...
			if ( selected ) then
				StockpileFrameFilters_AddSubSubCategories(subCategoryInfo.subCategories);
			end
		end
	end
end

function StockpileFrameFilters_AddSubSubCategories(subSubCategories)
...
function StockpileFilterButton_SetUp(button, info)
...
function StockpileFrameFilter_OnClick(self, button)
	if ( self.type == "category" ) then
		local wasToken = StockpileFrame_DoesCategoryHaveFlag("WOW_TOKEN_FLAG", StockpileFrameBrowse.selectedCategoryIndex);
	...
	StockpileBrowseLevelSort:SetText(StockpileFrame_GetDetailColumnString(StockpileFrameBrowse.selectedCategoryIndex, StockpileFrameBrowse.selectedSubCategoryIndex));
	-- BrowseWowTokenResults_Update();
	StockpileFrameFilters_Update(true)
	StockpileFrameBrowse_Search()
end

function StockpileFrameBrowse_Update()
	if (not StockpileFrame_DoesCategoryHaveFlag("WOW_TOKEN_FLAG", StockpileFrameBrowse.selectedCategoryIndex)) then
		...
		for i=1, NUM_BROWSE_TO_DISPLAY do
			...
			if ( not shouldHide ) then
				--{ id = object_id, sorted = object_info.name, name = object_info.name, h = object_info.h, q = object_info.q, t = object_info.texture, search_text = object_info.search_text, itemtype = object_info.itemtype, itemsubtype = object_info.itemsubtype, class = object_info.class }
				item_info =  ArkInventory.Search.SourceTable[index];
				item_info.name = item_info.name
				item_info.texture = item_info.t
				item_info.count = 1
				item_info.quality = item_info.q
				--item_info.canUse = StockpileItemIsUsable( item_info, i )
				item_info.level = item_info.uselevel
				item_info.levelColHeader = nil
				item_info.minBid = 1
				item_info.minIncrement = 1
				item_info.buyoutPrice = item_info.itemsubtypeid
				item_info.bidAmount = item_info.itemtypeid
				item_info.highBidder = true
				item_info.bidderFullName = "Mike"
				item_info.owner = "Merchant"
				item_info.ownerFullName = "Merchant Full"
				item_info.saleStatus = 0
				item_info.itemId = 1
				item_info.hasAllInfo = true
				...
				levelColHeader = item_info.levelColHeader
				minBid = item_info.minBid
				minIncrement = item_info.minIncrement
				buyoutPrice = item_info.buyoutPrice
				bidAmount = item_info.bidAmount
				highBidder = item_info.highBidder
				bidderFullName = item_info.bidderFullName
				owner = item_info.owner
				ownerFullName = item_info.ownerFullName
				saleStatus = item_info.saleStatus
				itemId = item_info.itemId
				hasAllInfo = item_info.hasAllInfo
			...
			if ( shouldHide ) then
				...
				if ( numBatchAuctions < NUM_BROWSE_TO_DISPLAY ) then
					...
					StockpileBrowseCurrentBidSort:SetWidth(207);
				elseif ( numBatchAuctions == NUM_BROWSE_TO_DISPLAY and totalAuctions <= NUM_BROWSE_TO_DISPLAY ) then
					...
					StockpileBrowseCurrentBidSort:SetWidth(184);
				end
				...
				-- Set closing time
				_G[buttonName.."ClosingTimeText"]:SetText(item_info.itemtype);
				_G[buttonName.."ClosingTime"].tooltip = item_info.itemtype;
				...
				highBidderFrame.Name:SetText(item_info.ilvl);
				...
				if ( GetSelectedAuctionItem("list") and (offset + i) == GetSelectedAuctionItem("list") ) then
					...
					if ( buyoutPrice > 0 and buyoutPrice >= minBid ) then
						...
						if ( canBuyout and (ownerName ~= UnitName("player")) ) then
							-- BrowseBuyoutButton:Enable();
							StockpileFrame.buyoutPrice = buyoutPrice;
						end
					else
						StockpileFrame.buyoutPrice = nil;
					end
					-- Set bid
...
-- function BrowseWowTokenResults_Update()
	-- if (StockpileFrame_DoesCategoryHaveFlag("WOW_TOKEN_FLAG", StockpileFrameBrowse.selectedCategoryIndex)) then
		...
		-- -- BrowseBidPrice:Hide();
		-- for i=1, NUM_BROWSE_TO_DISPLAY do
			-- local button = _G["StockpileBrowseButton"..i];
			-- button:Hide();
		-- end
		-- StockpileBrowseNoResultsText:Hide();
		-- StockpileBrowseNameSort:Hide();
		-- StockpileBrowseLevelSort:Hide();
		-- StockpileBrowseDurationSort:Hide();
		-- StockpileBrowseHighBidderSort:Hide();
		-- StockpileBrowseCurrentBidSort:Hide();
		-- StockpileBrowseSearchCountText:Hide();
		-- -- BrowsePrevPageButton.isEnabled = false;
		-- -- BrowsePrevPageButton:Disable();
		-- -- BrowseNextPageButton.isEnabled = false;	
		-- -- BrowseNextPageButton:Disable();
		-- FauxScrollFrame_Update(StockpileBrowseScrollFrame, 0, NUM_BROWSE_TO_DISPLAY, AUCTIONS_BUTTON_HEIGHT);
		-- local marketPrice;
...
-- function StockpileFrameBid_OnLoad(self)
	-- self:RegisterEvent("AUCTION_BIDDER_LIST_UPDATE");

	-- -- set default sort
	-- StockpileFrame_SetSort("bidder", "duration", false);
-- end

-- function StockpileFrameBid_OnEvent(self, event, ...)
	-- if ( event == "AUCTION_BIDDER_LIST_UPDATE" ) then
		-- StockpileFrameBid_Update();
	-- end
-- end

-- function StockpileFrameBid_OnShow()
	-- -- So the get auctions query is only run once per session, after that you only get updates
	-- if ( not StockpileFrame.gotBids ) then
		-- GetBidderAuctionItems();
		-- StockpileFrame.gotBids = 1;
	-- end
	-- StockpileFrameBid_Update();
-- end

-- function StockpileFrameBid_Update()
	...
	-- for i=1, NUM_BIDS_TO_DISPLAY do
		...
		-- else
			...
			-- -- Set closing time
			-- _G[buttonName.."ClosingTimeText"]:SetText(StockpileFrame_GetTimeLeftText(duration));
			-- _G[buttonName.."ClosingTime"].tooltip = StockpileFrame_GetTimeLeftTooltipText(duration);
			-- -- Set item texture, count, and usability
			-- iconTexture = _G[buttonName.."ItemIconTexture"];
			-- iconTexture:SetTexture(texture);
			-- if ( not canUse ) then
...
-- function BidButton_OnClick(button)
	...
	-- if ( GetCVarBool("auctionDisplayOnCharacter") ) then
		-- if ( not DressUpItemLink(GetSearchTableItemLink("bidder", button:GetID() + FauxScrollFrame_GetOffset(BidScrollFrame))) ) then
...
-- function StockpileFrameAuctions_OnLoad(self)
	...
	-- -- set default sort
	-- StockpileFrame_SetSort("owner", "duration", false);
-- end

-- function StockpileFrameAuctions_OnEvent(self, event, ...)
...
-- function StockpileFrameAuctions_OnShow()
	-- AuctionsTitle:SetFormattedText(AUCTION_TITLE, UnitName("player"));
	-- --MoneyFrame_Update("AuctionsDepositMoneyFrame", 0);
	-- AuctionsFrameAuctions_ValidateAuction();
	-- -- So the get auctions query is only run once per session, after that you only get updates
	-- if ( not StockpileFrame.gotAuctions ) then
		-- GetOwnerAuctionItems();
		-- StockpileFrame.gotAuctions = 1;
	-- end
	-- StockpileFrameAuctions_Update();
-- end

-- local AUCTIONS_UPDATE_INTERVAL = 0.5;
-- function StockpileFrameAuctions_OnUpdate(self, elapsed)
...
-- function StockpileFrameAuctions_Update()
	...
	-- for i=1, NUM_AUCTIONS_TO_DISPLAY do
		-- index = offset + i + (NUM_AUCTION_ITEMS_PER_PAGE * StockpileFrameAuctions.page);
		-- auction = _G["AuctionsButton"..i];
		-- -- Show or hide auction buttons
		-- if ( index > (numBatchAuctions + (NUM_AUCTION_ITEMS_PER_PAGE * StockpileFrameAuctions.page)) ) then
			-- auction:Hide();
			-- -- If the last button is empty then set isLastSlotEmpty var
		...
		-- else
			...
			-- if (index <= tokenCount) then
				...
				-- isWowToken = true;
				-- if (not name) then
					-- -- AuctionsWowTokenStockpileFrame:RegisterEvent("GET_ITEM_INFO_RECEIVED");
				-- end
			-- else
			...
			-- else
				...
				-- closingTimeText:SetText(StockpileFrame_GetTimeLeftText(duration));
				-- closingTimeFrame.tooltip = StockpileFrame_GetTimeLeftTooltipText(duration, isWowToken);
	...
	-- if ( pendingDeliveries ) then
		-- StockpileFrameAuctions:SetScript("OnUpdate", StockpileFrameAuctions_OnUpdate);
	-- else
		-- StockpileFrameAuctions:SetScript("OnUpdate", nil);
	-- end
...
-- function AuctionsButton_OnClick(button)
	...
	-- if ( GetCVarBool("auctionDisplayOnCharacter") ) then
		-- if ( not DressUpItemLink(GetSearchTableItemLink("owner", button:GetID() + effectiveIndex)) ) then
	...
	-- -- Close any auction related popups
	-- CloseAuctionStaticPopups();
	-- StockpileFrameAuctions.cancelPrice = button.cancelPrice;
	-- StockpileFrameAuctions_Update();
-- end

-- function PriceDropDown_OnShow(self)
	-- UIDropDownMenu_Initialize(self, PriceDropDown_Initialize);
	-- if ( not StockpileFrameAuctions.priceType ) then
		-- StockpileFrameAuctions.priceType = PRICE_TYPE_STACK;
	-- end
	-- UIDropDownMenu_SetSelectedValue(PriceDropDown, StockpileFrameAuctions.priceType);
-- end
...
-- function PriceDropDown_OnClick(self)
	-- if ( StockpileFrameAuctions.priceType ~= self.value ) then
		-- StockpileFrameAuctions.priceType = self.value;
...
-- function DurationDropDown_OnShow(self)
	...
	-- if ( not StockpileFrameAuctions.duration ) then
		-- StockpileFrameAuctions.duration = 2;
	-- end
	-- UIDropDownMenu_SetSelectedValue(DurationDropDown, StockpileFrameAuctions.duration);
-- end
...
-- function DurationDropDown_OnClick(self)
	-- StockpileFrameAuctions.duration = self.value;
	-- UIDropDownMenu_SetSelectedValue(DurationDropDown, self.value);
	-- UpdateDeposit();
-- end

-- function UpdateDeposit()
	-- MoneyFrame_Update("AuctionsDepositMoneyFrame", CalculateAuctionDeposit(StockpileFrameAuctions.duration, AuctionsStackSizeEntry:GetNumber(), AuctionsNumStacksEntry:GetNumber()));
-- end
...
-- function AuctionSellItemButton_OnEvent(self, event, ...)
	-- if ( event == "NEW_AUCTION_UPDATE") then
		...
		-- if (C_WowTokenPublic.IsAuctionableWowToken(itemID)) then
			...
			-- -- C_WowTokenPublic.UpdateTokenCount();
			-- -- AuctionsWowTokenStockpileFrame_Update();
			-- -- AuctionsWowTokenStockpileFrame:Show();
		...
		-- else
			...
			-- AuctionsWowTokenStockpileFrame:Hide();
			-- AuctionsItemButton:SetNormalTexture(texture);
...
-- function AuctionsFrameAuctions_ValidateAuction()
	...
	-- if ( C_WowTokenPublic.IsAuctionableWowToken(select(10, GetAuctionSellItemInfo()))) then
		-- -- AuctionsCreateAuctionButton:SetEnabled(not AuctionsWowTokenStockpileFrame.disabled and C_WowTokenPublic.GetCurrentMarketPrice());
		-- return;
	-- end
...
-- --[[
-- function StockpileFrame_UpdateTimeLeft(elapsed, type)
	...
	-- if ( self.updateCounter > AUCTION_TIMER_UPDATE_DELAY ) then
		...
		-- local index = self:GetID();
		-- if ( type == "list" ) then
			-- index = index + FauxScrollFrame_GetOffset(StockpileBrowseScrollFrame);
...
-- function StockpileFrame_GetTimeLeftText(id)
	-- return _G["AUCTION_TIME_LEFT"..id];
-- end

-- function StockpileFrame_GetTimeLeftTooltipText(id, isToken)
...
-- local function GetAuctionButton(buttonType, id)
	...
	-- elseif ( buttonType == "list" ) then
		-- return _G["StockpileBrowseButton"..id];
	-- end
-- end

function StockpileBrowseFrame_CheckUnlockHighlight(self, selectedType, offset)
...
-- function AuctionPriceTooltipFrame_OnLeave(self)
	-- StockpileBrowseFrame_CheckUnlockHighlight(self, "list", FauxScrollFrame_GetOffset(StockpileBrowseScrollFrame));
	-- GameTooltip_Hide();
-- end

function StockpileFrameItem_OnEnter(self, type, index)
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
	...
	if ( index <= 0 ) then
		...
		--GameTooltip:SetHyperlink(item_h)
...
-- function AuctionsWowTokenStockpileFrame_OnLoad(self)
	-- self:RegisterEvent("TOKEN_MARKET_PRICE_UPDATED");
	-- self:RegisterEvent("TOKEN_STATUS_CHANGED");
...
-- function AuctionsWowTokenStockpileFrame_OnEvent(self, event, ...)
	-- if (event == "TOKEN_MARKET_PRICE_UPDATED") then
		-- local result = ...;
		-- if (result == LE_TOKEN_RESULT_ERROR_DISABLED) then
			-- self.disabled = true;
...
-- function AuctionsWowTokenStockpileFrame_Update()
	-- local price, duration = C_WowTokenPublic.GetCurrentMarketPrice();
	-- if (WowToken_IsWowTokenAuctionDialogShown()) then
		-- price = C_WowTokenPublic.GetGuaranteedPrice();
	-- end
	-- if (price) then
		-- AuctionsWowTokenStockpileFrame.MarketPrice:SetText(GetMoneyString(price, true));
		-- local timeToSellString = _G[("AUCTION_TIME_LEFT%d_DETAIL"):format(duration)];
		-- AuctionsWowTokenStockpileFrame.TimeToSell:SetText(timeToSellString);
	-- else
		-- AuctionsWowTokenStockpileFrame.MarketPrice:SetText(TOKEN_MARKET_PRICE_NOT_AVAILABLE);
		-- AuctionsWowTokenStockpileFrame.TimeToSell:SetText(UNKNOWN);
	-- end
-- end

-- function AuctionWowToken_UpdateMarketPriceCallback()
	-- if (C_WowTokenPublic.GetCommerceSystemStatus() 
		-- and ((BrowseWowTokenResults:IsVisible() or AuctionsWowTokenStockpileFrame:IsVisible()) and not WowToken_IsWowTokenAuctionDialogShown())) then
		-- StockpileFrame.lastMarketPriceUpdate = GetTime();
		-- C_WowTokenPublic.UpdateMarketPrice();
	-- elseif (not (BrowseWowTokenResults:IsVisible() or AuctionsWowTokenStockpileFrame:IsVisible())) then
...
-- function AuctionWowToken_ShouldUpdatePrice()
	...
	-- elseif (not C_WowTokenPublic.GetCurrentMarketPrice()) then
		-- return true;
	-- elseif (not StockpileFrame.lastMarketPriceUpdate) then
		-- return true;
	-- elseif (now - StockpileFrame.lastMarketPriceUpdate > pollTimeSeconds) then
		-- return true;
	-- end
	-- return false;
-- end
...
-- function AuctionWowToken_UpdateMarketPrice()
	-- if (AuctionWowToken_ShouldUpdatePrice()) then	
		...
		-- C_WowTokenPublic.UpdateMarketPrice();
	-- end
	-- if ((BrowseWowTokenResults:IsVisible() or AuctionsWowTokenStockpileFrame:IsVisible()) and not WowToken_IsWowTokenAuctionDialogShown()) then
		-- local _, pollTimeSeconds = C_WowTokenPublic.GetCommerceSystemStatus();
		-- if (not StockpileFrame.priceUpdateTimer or pollTimeSeconds ~= StockpileFrame.priceUpdateTimer.pollTimeSeconds) then
			-- if (StockpileFrame.priceUpdateTimer) then
				-- StockpileFrame.priceUpdateTimer:Cancel();
			-- end
			-- StockpileFrame.priceUpdateTimer = C_Timer.NewTicker(pollTimeSeconds, AuctionWowToken_UpdateMarketPriceCallback);
			-- StockpileFrame.priceUpdateTimer.pollTimeSeconds = pollTimeSeconds;
		-- end
	-- end
-- end
...
-- function AuctionWowToken_CancelUpdateTicker()
	-- if (StockpileFrame.priceUpdateTimer) then
		-- StockpileFrame.priceUpdateTimer:Cancel();
		-- StockpileFrame.priceUpdateTimer = nil;
	-- end
-- end
...
-- function AuctionsCreateAuctionButton_OnClick()
	...
	-- else
		...
		-- local startPrice = MoneyInputFrame_GetCopper(StartPrice);
		-- local buyoutPrice = MoneyInputFrame_GetCopper(BuyoutPrice);
		-- if ( StockpileFrameAuctions.priceType == PRICE_TYPE_UNIT ) then
			-- startPrice = startPrice * AuctionsStackSizeEntry:GetNumber();
			-- buyoutPrice = buyoutPrice * AuctionsStackSizeEntry:GetNumber();
		-- end
		-- StartAuction(startPrice, buyoutPrice, StockpileFrameAuctions.duration, AuctionsStackSizeEntry:GetNumber(), AuctionsNumStacksEntry:GetNumber());
	-- end
-- end
